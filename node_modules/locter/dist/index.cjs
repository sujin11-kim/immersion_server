 ...
 */ /* istanbul ignore next */ function handleFileLoadError(e) {
...
const globAsync = node_util.promisify(glob);
...
function locateManySync(pattern, options) {
    ...
    for(let i = 0; i < patterns.length; i++){
        for(let j = 0; j < options.path.length; j++){
            const files = glob.sync(patterns[i], {
                absolute: true,
                cwd: options.path[j],
                nodir: true,
                ignore: options.ignore
...
function buildLoaderFilePath(input, withExtension) {
    let info;
    ...
    if (withExtension && info.extension) {
...
class ModuleLoader {
    ...
    async load(data, options) {
        ...
        if (options.withFilePrefix) {
            filePath = node_url.pathToFileURL(filePath).href;
        }
        try {
            // segmentation fault
            ...
            /* istanbul ignore next */ return handleFileLoadError(e);
        }
    }
...
exports.handleFileLoadError = handleFileLoadError;
